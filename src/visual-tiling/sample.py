...
def main(jsonl_file_path, output_dir, setting, target_difficulty, regex_path: str):
    ...
    for instance in instances:
        ...
        output_file_path = output_file_path.replace('/puzzles', '')
        ...
        try:
            ...
            print(f'failed to read output file {output_file_path}, {str(e)}')
            failed_list.append(output_file_path)
            continue
        ...
        print(f"  Visual Answer: {result['visual']['answer']} (Correct: {result['visual']['correct']})")
        
        if len(results) % 10 == 0:
            overall_correct = total_verbal_correct + total_visual_correct
            ...
            print(f"            Overall Correct: {overall_correct} (Accuracy: {overall_correct / completed_cnt * 100:.2f}%)")
            print('****************************************************************************') """
    ...
    overall_accuracy = (total_verbal_correct + total_visual_correct) / total_instances
    ...
    print(f"  Overall Correct: {total_verbal_correct + total_visual_correct} (Accuracy: {overall_accuracy * 100:.2f}%)")
